...
function createIconDefs() {
  const svg = document.createElementNS(SVG_NS, "svg");
  svg.setAttribute("class", "language-log-icon-defs");
  svg.setAttribute("aria-hidden", "true");
  svg.style.display = "none";
  svg.innerHTML = `
    <symbol id="language-log-icon-regex" viewBox="0 0 20 16" stroke="none" fill-rule="evenodd">
    ...
    <symbol id="language-log-icon-case" viewBox="0 0 20 16" stroke="none" fill-rule="evenodd">
...
class LogView {
  ...
  buildElement() {
    ...
    this.regexButton = createElement("button", { className: "btn" }, [
      createIcon("language-log-icon-regex"),
    ]);
    this.caseSensitiveButton = createElement("button", { className: "btn" }, [
      createIcon("language-log-icon-case"),
    ]);
    ...
    this.element = createElement("div", { className: "language-log-view", tabIndex: -1 }, [
      createIconDefs(),
      this.timestamps,
  ...
  handleConfigChanges() {
    this.disposables.add(
      atom.config.onDidChange("language-log.adjacentLines", () => {
        this.refreshTextFilter();
      }),
      atom.config.onDidChange("language-log.foldPosition", () => {
  ...
  destroyPanel() {
    const panel = atom.workspace.panelForItem(this.element);
    if (panel != null) {
      panel.destroy();
    }
